    ...
    fn consensus_decode<R: io::Read + ?Sized>(reader: &mut R) -> Result<Self, EncodeError> {
        ...
        let inner_keys: Vec<secp256k1::PublicKey> = (0..number_of_keys)
            .map(|_i| {
                ...
                secp256k1::PublicKey::deserialize(&bytes)
            })
...
impl Decodable for LightningCommitmentProof {
    fn consensus_decode<R: io::Read + ?Sized>(reader: &mut R) -> Result<Self, EncodeError> {
        ...
        let mut bytes = [0u8; PUBLIC_KEY_SIZE];
        reader.read_exact(&mut bytes)?;
        let revocation_pubkey = secp256k1::PublicKey::deserialize(&bytes)?;

        let to_self_delay: u16 = Decodable::consensus_decode(reader)?;

        let mut bytes = [0u8; PUBLIC_KEY_SIZE];
        reader.read_exact(&mut bytes)?;
        let local_delayed_pubkey = secp256k1::PublicKey::deserialize(&bytes)?;
...
impl Encodable for LightningHtlcData {
    fn consensus_encode<W: io::Write + ?Sized>(&self, writer: &mut W) -> Result<usize, io::Error> {
        ...
        len += writer.write(&self.revocation_key_hash.serialize())?;
        len += writer.write(&self.remote_htlc_key.serialize())?;
        len += writer.write(&self.local_htlc_key.serialize())?;
        len += writer.write(&self.payment_hash.serialize())?;
...
impl Decodable for LightningHtlcData {
    fn consensus_decode<R: io::Read + ?Sized>(reader: &mut R) -> Result<Self, EncodeError> {
        ...
        reader.read_exact(&mut bytes)?;
        let revocation_key_hash = hash160::Hash::deserialize(&bytes)?;

        let mut bytes = [0u8; PUBLIC_KEY_SIZE];
        reader.read_exact(&mut bytes)?;
        let remote_htlc_key = secp256k1::PublicKey::deserialize(&bytes)?;

        let mut bytes = [0u8; PUBLIC_KEY_SIZE];
        reader.read_exact(&mut bytes)?;
        let local_htlc_key = secp256k1::PublicKey::deserialize(&bytes)?;

        let mut bytes = [0u8; HASH_SIZE];
        reader.read_exact(&mut bytes)?;
        let payment_hash = hash160::Hash::deserialize(&bytes)?;
