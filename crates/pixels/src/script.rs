...
impl From<&ToLocalScript> for Script {
...
impl TryFrom<&Script> for ToLocalScript {
    ...
    fn try_from(script: &Script) -> Result<Self, Self::Error> {
        ...
        match (pubkeys.as_slice(), to_self_delay) {
            ([revocation_pubkey, local_delayed_pubkey], Some(to_self_delay)) => Ok(Self {
                revocation_pubkey: PixelKey(bitcoin::PublicKey::new(*revocation_pubkey)),
                to_self_delay,
                local_delayed_pubkey: *local_delayed_pubkey,
            }),
...
mod tests {
    ...
    fn test_simple_commitment_script_sig_parsing() -> Result<(), ToLocalScriptParseError> {
        ...
        let parsed_script = Script::from_str(SCRIPT_SIG).expect("Should always be valid");

        let to_local_script = ToLocalScript::try_from(&parsed_script)?;

        let _to_local_script_raw = Script::from(&to_local_script);
