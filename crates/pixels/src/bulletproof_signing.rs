...
pub fn tweak_signing_keys(
    ...
    let input_dh_key = ecdh(private_key, bulletproof.sender_key.to_public_key(), network)
    ...
    for (recipient, amount) in recipients {
        ...
        ) = get_commitment(private_key, recipient.to_public_key(), network, amount)?;
...
pub fn get_commitment(
    ...
    let dh_key = ecdh(private_key, public_key.to_public_key(), network)
        .map_err(|_e| BulletproofError::InvalidRangeProof)?;
...
pub fn create_signatures(
    ...
    let signature = ctx.sign_schnorr(
        &secp256k1::Message::from_hashed_data::<sha256::Hash>(&sha256::Hash::from_engine(engine)),
        &bitcoin::KeyPair::from_secret_key(ctx, &signing_key),
    );
    ...
    for (chroma, signing_key) in chroma_signing_keys {
        ...
        let chroma_signature = ctx.sign_schnorr(
            &secp256k1::Message::from_hashed_data::<sha256::Hash>(&sha256::Hash::from_engine(
                engine.clone(),
            )),
            &bitcoin::KeyPair::from_secret_key(ctx, signing_key),
        );
