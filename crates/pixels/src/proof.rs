...
pub struct P2WPKHWintessData {
    pub signature: Signature,
    pub pubkey: PublicKey,
}

impl P2WPKHWintessData {
...
impl From<P2WPKHWintessData> for Witness {
    fn from(value: P2WPKHWintessData) -> Self {
...
impl MultisigWintessData {
    pub fn new(signatures: Vec<Signature>, redeem_script: Script) -> Self {
        Self {
            signatures,
            redeem_script,
        }
    ...
    pub fn from_witness(
        ...
        let redeem_script = Script::from(unparsed_redeem_script.to_vec());
    ...
    /// The same as [`Self::into_witness`] but as [`Script`].
    pub fn to_witness_script(&self) -> Script {
        ...
        // Add redeem script
        script = script.push_slice(self.redeem_script.as_bytes());
...
impl MultisigPixelProof {
    ...
    /// other keys.
    pub(crate) fn create_multisig_redeem_script(&self) -> Result<Script, MultisigPixelProofError> {
        ...
        Ok(Script::new_multisig_redeem_script(&keys, self.m))
    }
    ...
    pub fn to_reedem_script(&self) -> Result<Script, MultisigPixelProofError> {
        self.create_multisig_redeem_script()
    }
}
...
impl From<LightningCommitmentWitness> for Witness {
    fn from(value: LightningCommitmentWitness) -> Self {
        ...
        witness.push(Script::from(&value.redeem_script).into_bytes());
...
impl LightningCommitmentProof {
    ...
    pub fn to_script_pubkey(&self) -> Result<Script, LightningCommitmentProofError> {
        let redeem_script = self.to_redeem_script()?;

        Ok(Script::from(&redeem_script).to_v0_p2wsh())
    }
}
