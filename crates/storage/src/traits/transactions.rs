...
const TXS_PREFIX_SIZE: usize = 4;
const TXS_PREFIX: &[u8; TXS_PREFIX_SIZE] = b"txs-";

/// Transaction storage key size is 4(`TXS_PREFIX:[u8; 4]`) + 32(`Txid`) = 36 bytes long
const TXS_STORAGE_KEY_SIZE: usize = TXS_PREFIX_SIZE + size_of::<Txid>();

fn tx_storage_key(txid: Txid) -> ByteArray<TXS_STORAGE_KEY_SIZE> {
    let mut bytes = [0u8; TXS_STORAGE_KEY_SIZE];

    bytes[..TXS_PREFIX_SIZE].copy_from_slice(TXS_PREFIX);
    bytes[TXS_PREFIX_SIZE..].copy_from_slice(&txid);
